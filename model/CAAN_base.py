...
# import importlib
# import model.cABN_model
# importlib.reload(model.cABN_model)
from model.component import attention_sigmoid, attention_distillation, fusion_module
...
class AttentionBranchNet_MobileNet(tf.keras.Model):
    ...
    def build(self, plot=False):
        ...
        inputs = Input(shape=self.shape, name="Attention_Inputs")
        ...
        if self.include_top:
            ...
            x = Reshape((self.n_class,),name="att_pred")(x)
...
class AttentionBranchNet_resnet(tf.keras.Model):
    ...
    def build(self):        
        ...
        inputs = Input(shape=self.img_shape, name="Attention_Inputs")
        x = inputs
        ...
        x    = GlobalAveragePooling2D()(x)
        pred = Dense(2, activation='softmax',name="att_pred")(x)
...
class ModelWithAttentionBranchBase:
    def __init__(self, att_shape = (256,256,3),
                       att_points = [],
        ...
        if base_model=="MobileNet":
            ...
            ValueError("Cannot find the base CAAN.")
        ...
        self.flat_method   = flat_method   # "gap", "conv"
        self.output_method = output_method # "separate", "merge"
